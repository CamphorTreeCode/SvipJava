...
<!--left二级下拉菜单  end-->
	...
	<div class="rightCenter">
     	 	 ...
     	 	 <section class="current">
     	 		...
     	 		<span class="weizhi">当前位置：退款管理 > 退款详情</span>
     	 	</section>
     	 	<!--当前位置结束-->
    </div>
			
	       <div class="rightBottom">
	       	<form action="${pageContext.request.contextPath }/admin/refund/refund" method="post" id="submitForm">
	       	    <input type="hidden" name="formId" value="${ refund.formId}">
	       	    <input type="hidden" name="token" value="${sessionScope.token }">
	       	    <input type="hidden" name="openId" value="${ refund.openId }">
	       	    <input type="hidden" name="refundId" value="${refund.refundId }">
	       	    <input type="hidden" name="refundState">  
	       		<div class="top">
				...
				<div class="main">
					...
					<input type="text" autocomplete="off"  class="zhanghao"  name="name" readonly="readonly" value="${refund.name }"/>
				</div>
				<div class="main">
					<label>电话</label>
					<!-- <input type="text" name="franchiseruserusername" id="name"   placeholder="请在此设置用户名(纯数字，长度为10个)"/> -->
					<input type="text" autocomplete="off"  class="username" readonly="readonly" name="phone" value="${refund.phone }" />
				</div>
				<div class="main">
					<label>退款金额</label>
					<!-- <input type="password" name="franchiseruserpassword" id="password"  placeholder="请在此设置密码(英文数字混合)"/> -->
					<input type="text" autocomplete="off" class="pwd" name="money" value="${refund.money }" />
				</div>
				<div class="main">
					<label>消费平台</label>
					<!-- <input type="password" name="franchiseruserpassword" id="password"  placeholder="请在此设置密码(英文数字混合)"/> -->
					<input type="text" autocomplete="off" class="pwd" readonly="readonly" name="platform" value="${refund.platform }" />
				</div>
				<div class="main">
					<label>申请退款时间</label>
					<input type="text" autocomplete="off" name="refundCreateTime" class="pid" value="${refund.refundCreateTime }" />
				</div>
				<div class="main">
					<div class="line_height">退款理由</div>
					<textarea type="text" autocomplete="off" name="refundReason" readonly="readonly" class="pid" >${refund.refundReason }</textarea>
				</div>
				
				<input type="hidden" autocomplete="off" name="openid" value="${openId}"/>
				<button id="btn" class="cancel se">不通过</button>
				<button class="save" type="submit">退款</button>
	     	 	...
	     	 	<script type="text/javascript">
	     	 		...
	     	 		$(function(){
	     	 		     ...
	     	 		     $("#btn").click(function(){
	     	 		        ...
	     	 		        var a = confirm("确定不通过吗？")
	     	 		        
	     	 		        if(!a){
	     	 		           return false;
	     	 		        }
	     	 		        
	     	 		        $("[name = refundState]").val(3)
	     	 		       
	     	 		        $("#submitForm").attr("action","${pageContext.request.contextPath }/admin/refund/noPass")
	     	 		      ...
	     	 		      $(".save").click(function(){
	     	 		        ...
	     	 		        var refundId =$("[name = refundId]").val()
	     	 		        
	     	 		        var openId = $("[name = openId]").val()
	     	 		        
	     	 		        var money = $("[name = money]").val()
	     	 		        
	     	 		        $.post("${pageContext.request.contextPath }/admin/refund/refund",{openId : openId, money : money, refundId :refundId, token : token},function(res){
	     	 		        
	     	 		               var data = JSON.parse(res)
	     	 		              if(data.state == "SUCCESS"){
	     	 		                 alert("退款成功")
	     	 		                 window.location.href="${pageContext.request.contextPath }/admin/refund/list"
	     	 		              }
	     	 		              else{
	     	 		                  
	     	 		                  alert("退款失败，错误信息:"+data.err_code_des)
