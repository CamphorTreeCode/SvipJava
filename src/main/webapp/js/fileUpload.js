...
var uploadTools = {
    ...
    "initWithSelectFile":function(opt){
        var uploadId = opt.uploadId;
//      $("#"+uploadId+" .article .selectFileBt").css("background-color","#0099FF");
        $("#"+uploadId+" .article .selectFileBt").off();
        $("#"+uploadId+" .article .selectFileBt").on("click",function(){
            if(opt.autoCommit){
                uploadEvent.cleanFileEvent(opt);
            }
       ...
       // "showFileItemProgress":false,
        ...
        "onUpload":onUploadFun,//在上传后执行的函数
         //autoCommit:true,//文件是否自动上传
          "fileType":['jpg','png','jpeg','bmp','gif','webp','ogg','mp4','mp3','avi'] //文件类型限制，默认不限制，注意写的是文件后缀
