$(function(){
		var fileMd5;
			...
			$(".btn").click(function(){
			   ...
			   if(uploadType==2){
				    ...
				    $tgaUpload2.on("uploadSuccess",function(file,respone){
	                 ...
	                 //全部上传完成触发事件
			        $tgaUpload2.on("uploadFinished",function(){
			        	f2=true;
			        	 if(f1 && f2){
			        		 $("form").submit();        
			        	 }
			...
			$(".qxbtn").click(function(){
				if($("[name=draftstate]").val()==0){
					window.location.href="admin/learningWorld/findLearningWorldList";
					 
				}
				if($("[name=draftstate]").val()==1){
					window.location.href="admin/learningWorld/findLearningWorldDraftList";
			...
			//页面回显
		       ...
		       var getFileBlob = function (url, cb) {
		 		...
		 		if(imgList!=""){
                  ...
                  //视频回显
			 	  var learningworldvideourl1=$("[name=learningworldvideourl1]").val();
			 	  var learningworldvideoimgurl1=$("[name=learningworldvideoimgurl1]").val();
			      var imgList1=[];
			 	  var imgList2=[];
			      imgList1.push(learningworldvideourl1)
			      imgList2.push(learningworldvideoimgurl1)
                  if(learningworldvideourl1!=""){
					...
					$(".diyUploadHover").remove();
					  ...
					  /*  $.each(imgList1, function(index,item){
				 			  getFileObject(item, function (fileObject) {
				 			    if(fileObject!=null){
				 			    var wuFile = new WebUploader.Lib.File(WebUploader.guid('rt_'),fileObject);
				 			    var file = new WebUploader.File(wuFile);
				 			    console.log("*********video**********")
				 			    console.log(file)
				 			    $tgaUpload1.addFiles(file)
				 			    }
				 			  })
				 			});
*/
					    
		 			$.each(imgList2, function(index,item){
		 			  getFileObject(item, function (fileObject) {
		 			    if(fileObject!=null){
		 			    var wuFile = new WebUploader.Lib.File(WebUploader.guid('rt_'),fileObject);
		 			    var file = new WebUploader.File(wuFile);
